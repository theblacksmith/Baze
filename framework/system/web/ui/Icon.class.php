...
class Icon extends HtmlComponent
{
	...
	function initialize(DOMElement $elem)
	{
		...
		System::$page->addCSS(LIB_ROOT . 'library/css/icon.css');
	}
	...
	protected function getEntireElement($preserveWhiteSpaces)
	{
		$xhtml = "";
		// Div
		$xhtml .= ($preserveWhiteSpaces ? _NL._NL : "" ) . '<div ';
		$xhtml .= $this->getPropertiesList() . '>';
		// Img
		$xhtml .= ($preserveWhiteSpaces ? _NL : "" ) . $this->image->getXHTML(XML_PART_ENTIRE_ELEMENT, false);
		// Input
		$xhtml .= ($preserveWhiteSpaces ? _NL : "" ) . $this->text->getXHTML(XML_PART_ENTIRE_ELEMENT, false);
