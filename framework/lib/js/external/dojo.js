...
/*
 *	Cross-browser function to create a DOM Document
 *
 *	Modified by Saulo
 *	CHANGES: added possibility to pass parameters supported by Mozilla's createDocument
 */

dojo.dom.createDocument = function(nsURI, docRootTag, docType){
	var doc = null;
	var prefix = "a0";
	var openTag = docRootTag;
	var closeTag = docRootTag;

	if(!dj_undef("ActiveXObject"))
	{
		var activeX = [ "MSXML4.DOMDocument",
                   		"MSXML3.DOMDocument",
                   		"MSXML2.DOMDocument",
                   		"MSXML.DOMDocument",
                   		"Microsoft.XmlDom"];
		...
		for(var i = 0; i < activeX.length; i++){
			try{
				doc = new ActiveXObject(activeX[i]);
			}catch(e){ /* squelch */ };
		...
		if(nsURI != "") {
			openTag = prefix + ":" + docRootTag + ' xmlns:a0="' + nsURI + "'";
			closeTag = prefix + ":" + docRootTag;
		}

		if(docRootTag != "")
			doc.loadXML('<' + openTag + '></' + closeTag + '>');
	}
	else if((document.implementation)&& (document.implementation.createDocument))
	{
		doc = document.implementation.createDocument(nsURI, docRootTag, docType);
	}
