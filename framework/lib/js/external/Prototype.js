...
if(typeof Base != "undefined") {
	Base.provide("external.Prototype");
}
...
Function.prototype.bindEnvironment = function() {
	var __method = this, args = $A(arguments), object = args.shift(), env = args.shift();
  	  	
	for(var v in env)
	{
	  	eval.apply(object,"var " + v + " = object['"+v+"'];");
	}
		
	return function() {	
	    	return __method(args.concat($A(arguments)));
			  };
};
...
is_array = function is_array(o) {
	return (o instanceof Array || typeof o == "array" || (o["constructor"] && o.constructor.name == 'array'));
};

Object.extend(Array.prototype, {
	/* Added by Saulo Vallory */
	init: function(zero) {
...
if (!window.Element) {
  Element = Prototype.emptyFunction;
}
...
Object.extend(Element.prototype, {
  ...
  /* Removido porque n?o est? sendo usado e Buga o IE
	  inspect: function() {
			var str = "";
			...
			if(this.hasAttributes()) {
				...
				{
					if(this.attributes.item(i).nodeValue !== "" || printEmptyAttributes === true) {
						str += " " + this.attributes.item(i).inspect(); //" " + this.attributes.item(i).nodeName + '="' + this.attributes.item(i).nodeValue + '"';
					}
				}
			}
			...
			if(this.hasChildNodes()) {
				...
				{
					try{
						str += this.childNodes.item(i).inspect();
					}
					catch(e)
					{
						alert("inspect() not exists for " + this.childNodes.item(i) + ". Skipping.");
					}
				}
...
if(!Element.prototype.getAttribute)
{
	Object.extend(Element.prototype, {
		getAttribute: function (attribute) {
			...
			if(this.attributes[attribute] && typeof this.attributes[attribute].nodeValue != "undefined")
		...
		setAttribute: function (attribute, value) {
			...
			if(typeof this.attributes[attribute] == "undefined")
				this.attributes[attribute] = document.createAttribute(attribute);
				
			this.attributes[attribute].nodeValue = value;
	  	},

	  	hasAttribute: function (attribute, value) {
	    	if (attribute == "class"){ attribute = "className"; }
	    	if (attribute == "for"){ attribute = "htmlFor"; }
	  	...
	  	removeAttribute: function (attribute) {
			...
			var remAtt = this.attributes[attribute];
...
if(typeof Node == "undefined")
{
	...
	// Adding transparent behaviour to .png images
	// and DOM behaviour to nodes
/*
	var ss2 = document.createStyleSheet();
	ss2.cssText= "IMG {" +
				"	behavior: url(" LIB_ROOT + "library/js/system/pngBehaviour.htc) url(" + LIB_ROOT + "library/js/system/ieBehaviours.htc);" +
				"}";
	*/
}