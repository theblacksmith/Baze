...
{
	dojo.require("external.Prototype");
	dojo.provide("web.components.panels");
...
if (!window.ActiveXObject)
{
	function TabGroup(id)
	{
		...
		for (var i = 0; i < this.tabs.length; i++) {
			...
			if (!dojo.html.hasClass(d, 'panel') || d.parentNode.getAttribute('phpclass') == null || d.parentNode.getAttribute('phpclass') != 'php:panelcontainer') {skipped++;i--;continue;}
			this.pans[i] = d;
	...
	Baze.addBehaviour(findTabGroups);
}